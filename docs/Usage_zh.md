...
| 模式 | 行为 | 什么时候用 |
|---|---|---|
| `--sched=inorder` | effect 立即生效；每条 op 后轮换可运行核 | 确定性数值结果，但**按构造原理发现不了任何同步问题** |
| `--sched=lazy`（默认） | effect 尽可能晚提交；每条 op 后轮换可运行核 | 日常检查，并稳定模拟多核并行推进 |
| `--sched=fuzz --seed=N` | lazy + 随机选核 + 1～8 条 op 的随机时间片 | 找那些"换个调度就崩"的脆弱同步 |
